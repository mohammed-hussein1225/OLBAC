...
document.addEventListener("DOMContentLoaded", function () {
  function cart() {
    ...
    icons.forEach((icon) => {
      icon.addEventListener("click", function () {
        ...
        const priceValue = parentIcons.dataset.price;
            ...
            <div class="container">
              ...
              <div class="row">
                <div class="col-md-2 d-flex align-items-center justify-content-center">
                  <img src="${imageValue}" width="80%" />
                </div>
                <div class="col-md-4 pt-3 align-items-center justify-content-center">
                  <p class="fw-bold">Godox XPro II TTL Wireless Flash Trigger for Sony Cameras</p>
                  <div class="cont d-flex align-items-center gap-5 justify-content-between">
                    ...
                    <div class="item">
                      ...
                      <b href="#" style="cursor:pointer" class="remove-item" data-image="${imageValue}" data-price="${priceValue}">remove</b>
                    </div>
                  </div>
                </div>
                <div class="col-md-2 pt-3 text-center" style="border-right: 1px solid #ced5e1">
                  <label for="inp" class="d-inline-block">Quantity: </label>
                  <input type="number" name="inp" class="w-25 d-block mt-2" style="margin: 0 auto" value="1" />
                </div>
                <div class="col-md-2 text-center my-auto" style="border-right: 1px solid #ced5e1">
                  <p class="price_1" style="color: #007594; font-weight: bold; font-size: 13pt; white-space: nowrap;">${priceValue}</p>
                  <p class="price_2" style="color: #898989; font-size: 11pt; white-space: nowrap; text-decoration: line-through !important;">${priceValue}</p>
                </div>
                <div class="col-md-2 text-center my-auto">
                  <p class="sup-total" style="color: #007594; font-weight: bold; font-size: 13pt; white-space: nowrap;">${priceValue}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        // استرجاع البيانات المخزنة سابقًا
        let savedCart = JSON.parse(localStorage.getItem("cartContent")) || [];

        // إضافة العنصر الجديد
        savedCart.push(cart_content);
  ...
  function displayCart() {
    ...
    if (cartBox) {
      cartBox.innerHTML = savedCart.join(""); // عرض كل العناصر داخل `cart-box`
      setupRemoveButtons(); // تفعيل أزرار الحذف بعد العرض
    }
  }
    ...
    document.querySelectorAll(".remove-item").forEach((btn) => {
      btn.addEventListener("click", function () {
        ...
        savedCart = savedCart.filter(
          (item) => !item.includes(imageValue) || !item.includes(priceValue)
        );
  ...
  // استعادة محتوى السلة عند تحميل الصفحة
  displayCart();

  // تفعيل وظيفة السلة عند تحميل الصفحة
  cart();
  function updateTotalPrice() {
    const total_price = document.querySelectorAll(".cart .sup-total");

    let totals = 0;
    total_price.forEach((total) => {
      totals += Number(total.innerHTML);
    });
    if (!document.querySelector(".total .total_price")) return;
    document.querySelector(".total .total_price").innerHTML = totals;
  }
